...
import { InvitationTimeline } from "./invitation-timeline";
...
import { timelineConfigSchema } from "../schemas/timeline.schema";
...
export function InvitationSectionRenderer({
  ...
  switch (section.type) {
    ...
    case "RSVP": {
      if (config.variant === "botanical-editorial") {
        return <RsvpBotanicalEditorial weddingId={invitation.slug} />;
      }
    ...
    case "TIMELINE": {
      const config = timelineConfigSchema.parse(section.config ?? {});
      return <InvitationTimeline config={config} media={invitation.media} />;
    }
